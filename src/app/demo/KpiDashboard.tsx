    ...
    return (
      <div>
        ...
        <div className={`grid ${fullWidth ? "grid-cols-4" : "grid-cols-2"} gap-2`}>
          <KPICard label="Marge" value={fmtPct(costData.marge)} color={costData.marge > 0 ? "#22C55E" : "#EF4444"} info={{ definition: "Gewinnspanne bezogen auf die Gesamtkosten.", formula: "(Verkaufserlös − Gesamtkosten) ÷ Gesamtkosten" }} />
          <KPICard label="IRR (ann.)" value={fmtPct(costData.irrSell)} color="#0D9488" info={{ definition: "Annualisierte Rendite des Verkaufsszenarios.", formula: "Annualisierte Marge über Projektlaufzeit" }} />
          ...
          <KPICard label="Break-Even" value={costData.breakEvenMonth !== null && costData.breakEvenMonth !== undefined ? `${costData.breakEvenMonth}` : "—"} unit=" Mo." color="#22C55E" info={{ definition: "Monat, ab dem das Projekt kumuliert im Plus ist.", formula: "Erster Monat mit kumuliertem CF ≥ 0" }} />
          <KPICard label="Max. Kapitalbedarf" value={fmtEur(Math.abs(costData.peakCapital ?? 0))} color="#EF4444" info={{ definition: "Höchster Punkt der negativen Cashflow-Kurve.", formula: "Min(kumulierter Cashflow)" }} />
          <KPICard label="Grundstücksanteil" value={fmtPct(costData.grundstuecksanteil)} color="#FBBF24" info={{ definition: "Anteil der Grundstückskosten an den Gesamtkosten.", formula: "KG100 ÷ Gesamtkosten" }} />
          <KPICard label="Baukosten/m²" value={`${Math.round(costData.baukostenProM2).toLocaleString("de-DE")}`} unit=" €" color="#A78BFA" info={{ definition: "Reine Baukosten (KG300) pro m² Bruttogrundfläche.", formula: "KG300 ÷ BGF" }} />
          <KPICard label="Betrachtung" value={`${costData.gesamtlaufzeit}`} unit=" Mo." color="#94A3B8" info={{ definition: "Gesamter Analysezeitraum des Projekts in Monaten." }} />
          {finanzierungAktiv && (
            <KPICard label="Monatl. Rate" value={fmtEur(costData.monatlicheRate)} color="#F59E0B" info={{ definition: "Monatliche Annuitätenrate (Zins + Tilgung).", formula: "FK × (Zinssatz + Tilgung) ÷ 12" }} />
          )}
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      <div className={`grid ${fullWidth ? "grid-cols-4" : "grid-cols-2"} gap-2`}>
        ...
        />
        <KPICard label="Total Profit" value={fmtEur(totalProfit)} color={totalProfit > 0 ? "#22C55E" : "#EF4444"} info={{ definition: "Absoluter Gewinn über die gesamte Haltedauer.", formula: "Summe aller Cashflows" }} />
        <KPICard label="Avg. Cash Yield" value={fmtPct(avgCY)} color={avgCY > 8 ? "#22C55E" : avgCY >= 4 ? "#FBBF24" : "#EF4444"} info={{ definition: "Durchschnittlicher jährlicher Ertrag auf dein EK (ohne Exit).", formula: "Ø Jahres-Cashflow ÷ EK" }} />
        <KPICard label="Nettoanfangsrendite" value={fmtPct(costData.niy)} color="#0D9488" info={{ definition: "Jährliche Nettomiete im Verhältnis zum Gesamtinvestment.", formula: "Nettomiete p.a. ÷ Investitionskosten" }} />
        ...
        {finanzierungAktiv && (
          <>
            <KPICard label="Restschuld" value={fmtEur(restschuld)} color="#F59E0B" info={{ definition: "Verbleibende Kreditschuld am Ende des Betrachtungszeitraums.", formula: "FK − kumulierte Tilgung" }} />
            <KPICard label="Equity Build-up" value={fmtEur(eqBuildup)} color="#22C55E" info={{ definition: "Durch Tilgung aufgebautes Eigenkapital im Objekt.", formula: "FK-Volumen − Restschuld" }} />
          </>
        )}
        <KPICard label="Monatl. Rate" value={fmtEur(costData.monatlicheRate)} color="#F59E0B" info={{ definition: "Monatliche Annuitätenrate (Zins + Tilgung).", formula: "FK × (Zinssatz + Tilgung) ÷ 12" }} />
        <KPICard label="Nettomiete/Mo." value={fmtEur(nettomieteJahr / 12)} color="#22C55E" info={{ definition: "Monatliche Mieteinnahme nach Bewirtschaftungskosten.", formula: "Jahresmiete × (1 − Bewirtschaftung%) ÷ 12" }} />
        <KPICard
